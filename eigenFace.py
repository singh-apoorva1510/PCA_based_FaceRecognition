...
# All images are supposed to be of the same size, say, N*L
IMAGE_DIR = "/home/apoorva/projects/Face-recognition-using-eigenfaces-master/training_data"
...
class EigenFace(object):
    ...
    def __init__(self, image_path=IMAGE_DIR,suffix="*.*",variance_pct=0.99,knn=5):
        ...
        for idx,image_name in enumerate(image_names):
            ...
            if idx == 0:
                # the shape of the image. They are supposed to be the same
    ...
    def get_eigen(self):
        ...
        shape = self.vector_matrix.shape
         # if there is less number of training images. Usually go for 'else' branch.
        if (shape[0]<shape[1]):
    ...
    def plot_eigen_value_distribution(self):
        ...
        axarr.set_title("PCA Components Vs Variance")
        data = np.cumsum(self.eigen.lamb,) / np.sum(self.eigen.lamb)
